...
def limpiar_datos_nulos(df, variables_requeridas):
    ...
    if nulos_por_columna:
        ...
        print(f"   ✓ Registros válidos: {len(df_limpio):,} ({len(df_limpio)/registros_originales*100:.1f}%)")
    else:
...
def validar_columnas_requeridas(df, variables_requeridas):
    ...
    columnas_faltantes = [var for var in variables_requeridas if var not in columnas_presentes]
...
def validar_valores(df, valores_validos, rangos_validos):
    ...
    for var, valores in valores_validos.items():
        if var in df.columns:
            ...
            if invalidos:
                errores.append(f"Valores inválidos en {var}: {invalidos}. Válidos: {valores}")
                pass
    ...
    # MELÓN, SANTA ANA, MELON, EL VALLE, ESTRELLA
    if errores:
...
def mostrar_ejemplos(df, n=10):
    ...
    columnas_mostrar = [
        'nombre_sector', 'nro_crianza', 'Pabellón', 'sexo',
        'kilos_recibidos_percapita', 'tipoConstruccion',
        'densidad_pollos_m2', 'ganancia_proyectada'
    ]
...
def main():
    ...
    try:
        ...
        # MELÓN, SANTA ANA, MELON, EL VALLE, ESTRELLA
        df = df[~df['nombre_sector'].isin(['MElÓN', 'SANTA ANA', 'EL VALLE', 'ESTRELLA'])]
        ...
        df['tipoConstruccion'] = np.where(df['tipoConstruccion'] == 'Sin Información', 'Black Out', df['tipoConstruccion'])
        df['densidad_pollos_m2'] = np.where(df['densidad_pollos_m2'] < 0, 14.5, df['densidad_pollos_m2'])
        ...
        print(f"   • Ganancia promedio proyectada: {df_resultado['ganancia_proyectada'].mean():.2f}g")
